...
class RoomDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['total_imgs'] = RoomTypeImages.objects.filter(room_id=self.kwargs.get('pk')).count()
        context['front_display'] = RoomTypeImages.objects.filter(room_id=self.kwargs.get('pk'))[0:1]
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if number_of_guests > room.capacity:
                ...
                messages.error(request, f'({number_of_guests} people) cannot book this room because is ({room.room_type})')
...
def verify_booking(request, ref):
    ...
    if verified:
        ...
        amount = (hostel_fee - float(50)) * 100
        ...
        try:
            ...
            messages.error(request, f'{e}, Instant cash transfer could not be initiated. We will manually transfer payment to your landlord.')
...
class CreateHostel(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            #form.instance.hostel_amenities = dict(item.split('=') for item in form.cleaned_data['amenities'].split(','))#form.cleaned_data['amenities']
...
class RoomTypeCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        #form.instance.cost_per_head = float(form.cleaned_data['cost_per_head']) + (float(form.cleaned_data['cost_per_head']) * 0.04)
        form.instance.cost_per_head = float(form.cleaned_data['cost_per_head']) + float(50)
...
class HostelUpdate(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse('room-detail', kwargs={'pk': self.object.pk, 'hostel':self.object.hostel_name})
