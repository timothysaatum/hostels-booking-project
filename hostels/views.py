...
class HostelListView(ListView):
    ...
    def get_queryset(self):
        ...
        try:
            ...
            return Hostel.active.all().select_related('school').order_by('-is_featured', '-created_at')
    ...
    def _calculate_and_add_distances(self, context):
        ...
        if not (school_id and school_id.isdigit() and hasattr(context['hostels'], '__iter__')):
            # No school selected or no hostels - set default values
            for hostel in context['hostels']:
                hostel._calculated_distance = None
            ...
            for hostel in context['hostels']:
                if hostel.latitude and hostel.longitude:
                    ...
                    if distance is not None:
                        hostel._calculated_distance = distance
    ...
    def _apply_distance_sorting(self, context):
        ...
        if sort_by == 'distance' and hasattr(context['hostels'], '__iter__'):
            ...
            hostels_list.sort(key=lambda h: (
                getattr(h, '_calculated_distance', None) is None,  # None values go to end
                getattr(h, '_calculated_distance', None) if getattr(h, '_calculated_distance', None) is not None else float('inf')
            ))
...
class HostelDetailView(DetailView):
    ...
    def get_queryset(self):
        return (
            ...
            .prefetch_related(
                ...
                "images",
                Prefetch(
                    "room_types", queryset=RoomType.objects.prefetch_related("images")
                ),
            )
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        if (
            hostel.latitude
            and hostel.longitude
            and hostel.school.latitude
            and hostel.school.longitude
        ):
            distance = calculate_distance(
                hostel.school.latitude,
                hostel.school.longitude,
                hostel.latitude,
                hostel.longitude,
            )
        ...
        similar_hostels = (
            Hostel.active.all().filter(school=hostel.school).exclude(id=hostel.id)[:4]
        )
...
class HostelCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, "Hostel created successfully! Now add your room types."
        )
        return super().form_valid(form)

    def get_success_url(self):
        return (
            reverse("hostels:roomtype-create", kwargs={"hostel_slug": self.object.slug})
            + "?new_hostel=1"
        )
...
class RoomTypeCreateView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        with transaction.atomic():
            ...
            self.object = (
                form.save()
            )  # This will automatically create rooms via the save method
    ...
    def get_success_url(self):
        ...
        if is_new_hostel:
            return (
                reverse(
                    "hostels:roomtype-create", kwargs={"hostel_slug": self.hostel.slug}
                )
                + "?new_hostel=1&continue=1"
            )
        else:
...
class RoomTypeUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def _adjust_rooms(self, new_total, original_total):
        ...
        if new_total > current_rooms:
            ...
            rooms_to_delete = self.object.rooms.filter(current_occupants=0).order_by(
                "-created_at"
            )[:excess_rooms]
    ...
    def get_success_url(self):
        return reverse(
            "hostels:hostel-detail", kwargs={"slug": self.object.hostel.slug}
        )
...
class RoomTypeDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(
            request, "Room type and all associated rooms deleted successfully!"
        )
        return super().delete(request, *args, **kwargs)
...
class DashboardView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["new_inquiries"] = ContactInquiry.objects.filter(
            hostel__owner=self.request.user, status="new"
        ).count()
...
def mark_inquiry_contacted(request, inquiry_id):
    ...
    inquiry = get_object_or_404(
        ContactInquiry, id=inquiry_id, hostel__owner=request.user
    )
...
def finish_hostel_setup(request, hostel_slug):
    ...
    if hostel.room_types.count() == 0:
        messages.warning(
            request, "Please add at least one room type to complete your hostel setup."
        )
    ...
    if request.method == "POST":
        try:
            ...
            if latitude and longitude:
                ...
                return JsonResponse(
                    {"success": False, "message": "Invalid coordinates received."}
                )
...
def hostel_stats_api(request):
    ...
    if not stats:
        stats = {
            ...
            "total_rooms": sum(hostel.total_rooms for hostel in Hostel.active.all()),
            "average_price": Hostel.active.all().aggregate(
                avg_min=models.Avg("min_price"), avg_max=models.Avg("max_price")
            ),
        }
